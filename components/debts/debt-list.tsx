    ...
    <ul className="divide-y divide-border">
      {debts.map((debt) => (
        <PlanningRow
          ...
          title={
            <>
              {/* The person's own name is its own element, distinct from the
                  direction word beside it — so a reader (and a test) can find
                  "Minh" on its own, not only "MinhOwes you" run together. */}
          ...
          progress={
            !compact && (
              <Progress
                // The bar is `tone="positive"` in BOTH directions: it measures
          ...
          extra={
            !compact && (
              <>
                ...
                {debt.notes && <p className="text-xs/[1rem] text-muted-foreground">{debt.notes}</p>}
